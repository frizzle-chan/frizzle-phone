...
# ---------------------------------------------------------------------------
# Sound generators — all return list[float] normalised roughly to [-1, 1]
# ---------------------------------------------------------------------------
...
def _gen_kick(duration_s: float = 0.08) -> list[float]:
...
def _gen_snare(duration_s: float = 0.10) -> list[float]:
...
def _gen_reese_note(freq: float, duration_s: float) -> list[float]:
...
def _float_to_ulaw_buf(samples: list[float], peak: float = 0.95) -> bytes:
...
def _generate_bass_pattern(
    sixteenth_samples: int, num_measures: int = 4
) -> list[float]:
    ...
    while pos < total_sixteenths:
        ...
        if random.random() < probs[slot]:
            ...
            note = _gen_reese_note(freq, dur_s)
            _mix_into(bass_track, note, pos * sixteenth_samples, 1.0)
            pos += note_len
        else:
            pos += 1
...
def generate_rhythm(duration_s: float = 60.0) -> bytes:
    ...
    for beat in range(4):
        ...
        if beat % 2 == 0:
            _mix_into(drum_measure, kick, on_offset, 1.0)
        else:
            _mix_into(drum_measure, snare, on_offset, 0.9)
        _mix_into(drum_measure, hihat, on_offset, 0.6)

        # Off-beat: open hi-hat
        _mix_into(drum_measure, open_hat, off_offset, 0.55)
