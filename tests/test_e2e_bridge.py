...
from tests.audio_helpers import FIXTURES, pcm_to_wav, wav_samples_check
from tests.fake_voice import (
    CHORD_FREQS,
    FakeVoiceRecvClient,
    sine_tone_speakers,
    wav_file_speakers,
...
# --- WAV-file speaker helpers ---


async def _run_bridge_test(
    discord_db: aiosqlite.Connection,
    speakers: dict[int, list[np.ndarray]],
    file_regression,
    *,
    call_id: str = "e2e-wav",
) -> None:
    """Common SIP flow: INVITE → bridge → collect RTP → golden WAV → BYE."""
    loop = asyncio.get_running_loop()
    max_ticks = max((len(f) for f in speakers.values()), default=0)
    ...
    try:
        sip_client_transport.sendto(
            _build_invite(server_port, client_port, rtp_port, call_id=call_id)
        )
        ...
        sip_client_transport.sendto(
            _build_ack(server_port, client_port, call_id=call_id)
        )

        max_wait = max_ticks * 25 + 5000  # 25ms/tick + 5s buffer
        for _ in range(max_wait // 10):
            await asyncio.sleep(0.01)
            if len(collector.packets) >= max_ticks:
                break

        assert len(collector.packets) >= max_ticks, (
            f"Only received {len(collector.packets)}/{max_ticks} RTP packets"
        )

        received_ulaw = b""
        for pkt in collector.packets[:max_ticks]:
            received_ulaw += parse_rtp_payload(pkt)
        ...
        sip_client_transport.sendto(
            _build_bye(server_port, client_port, call_id=call_id)
        )
        bye_resp = parse_message(await _recv(sip_proto.queue))
        assert bye_resp.uri == "200"
...
def _conversation_offsets(paths: list, overlap: int = 25) -> list[int]:
    """Stagger offsets so each clip starts ~overlap ticks before the previous ends."""
    offsets = [0]
    for i in range(1, len(paths)):
        with wave.open(str(paths[i - 1]), "rb") as wf:
            n_48k = wf.getnframes() * 48000 // wf.getframerate()
        prev_ticks = (n_48k + 959) // 960
        offsets.append(offsets[i - 1] + prev_ticks - overlap)
    return offsets


# --- WAV-file speaker tests ---


@pytest.mark.asyncio
async def test_solo_speaker_bridge(
    discord_db: aiosqlite.Connection, file_regression
) -> None:
    """Single WAV speaker through the full bridge pipeline."""
    speakers = wav_file_speakers([FIXTURES / "speech_sample.wav"])
    await _run_bridge_test(discord_db, speakers, file_regression, call_id="e2e-solo")


@pytest.mark.asyncio
async def test_simultaneous_speakers_bridge(
    discord_db: aiosqlite.Connection, file_regression
) -> None:
    """5 WAV speakers all talking simultaneously from tick 0."""
    paths = [FIXTURES / f"joe_{i}.wav" for i in range(1, 6)]
    speakers = wav_file_speakers(paths)
    await _run_bridge_test(
        discord_db, speakers, file_regression, call_id="e2e-simultaneous"
    )


@pytest.mark.asyncio
async def test_conversation_bridge(
    discord_db: aiosqlite.Connection, file_regression
) -> None:
    """12-clip turn-taking conversation with ~25 tick overlap between turns."""
    names = []
    for i in range(1, 7):
        names.extend([f"joe_{i}.wav", f"kathleen_{i}.wav"])
    paths = [FIXTURES / name for name in names]
    offsets = _conversation_offsets(paths)
    speakers = wav_file_speakers(paths, offsets=offsets)
    await _run_bridge_test(
        discord_db, speakers, file_regression, call_id="e2e-conversation"
    )
